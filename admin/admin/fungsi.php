...
if (!function_exists('editart'))   {
	function editart($data) {
		global $conn;

	  $idart = htmlspecialchars($data["id_artikel"]);
	  $isi= htmlspecialchars($data["artikel"]);

	    $sql = "UPDATE artikel SET isiart= '$isi' WHERE id_artikel = $idart";
